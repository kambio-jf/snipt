...
const MARKER_SEQUENCES = [
  ...
  ["key", "board", "clicking"],
  ["s", "ound"],
  // NOT a bare ["sound"]: this list matches whole normalised tokens, so that
  // entry deleted the word wherever it was genuinely spoken. The corpus has one
  // real "sound" in it — Joel saying "that's a sound rate" on 2026-07-14 — and
  // the entry silently ate it. Same reasoning the [TYPING]/[SNIFF] note below
  // spells out; this line predated that note and contradicted it.
  // 2026-09-02: [TYPING] and [SNIFF], brackets stripped the same way. Only the
  // SPLIT forms go here on purpose — a bare "typing" or "sniff" is a word Joel
  // says for real ("I can't type today"), and the list is matched on normalised
  // alphanumerics, so a single-token entry would eat the real word too.
  ["ty", "ping"],
  ["sn", "iff"],
];
