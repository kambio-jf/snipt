...
`);
    try {
      ...
      // Say WHY. On 2026-09-10 this fired on a 53-minute master and the message
      // could not distinguish an ffmpeg failure from the rename failing because
      // the file was open elsewhere (it was being uploaded at that moment, and
      // Windows refuses to rename over an open file). Those need opposite fixes.
      const why = err.code ? `${err.code}${err.syscall ? ` on ${err.syscall}` : ""}` : String(err.stderr || err.message || err).trim().split(String.fromCharCode(10)).pop();
      console.log(`   ⚠ ${label} correction failed at ceiling ${limit} (${why}) — file left at ${m.i.toFixed(1)} LUFS / ${m.tp.toFixed(2)} dBTP, fix by hand`);
      if (err.code === "EBUSY" || err.code === "EPERM") console.log(`     the file is probably open in another program (an upload, a player) — close it and re-run the level`);
