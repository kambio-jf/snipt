...
export const YT = dirname(fileURLToPath(import.meta.url));      // yt-tools dir (has models/)
...
export function runWordWhisper(clip) {
  ...
  const destRel = relative(YT, tmp).replace(/\\/g, "/");
  execFileSync("ffmpeg", ["-y", "-i", clip, "-vn", "-af",
    ...
    "-f", "null", "-"], { stdio: ["ignore", "ignore", "inherit"], cwd: YT });
  ...
  const cf = join(YT, "corrections.json");
